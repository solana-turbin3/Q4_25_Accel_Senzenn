...
pub struct TransferHook<'info> {
    ...
    // The PDA will be derived as [b"whitelist", owner.key()]
    // SPL's account resolution will automatically provide the correct PDA for each user
    #[account(
...
impl<'info> TransferHook<'info> {
    ...
    pub fn transfer_hook(&mut self, _amount: u64) -> Result<()> {
        ...
        // For per-user PDAs, the account existence itself proves the user is whitelisted
        // If the PDA doesn't exist, Anchor's constraint validation will fail the transaction
        // No need for manual checking - the account resolution handles it
        Ok(()) // If we reach here, the user is whitelisted (their PDA exists)
    }
